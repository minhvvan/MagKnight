...
public class EnemyHitDetector : MonoBehaviour, IObservable<GameObject>
{
    [Serializable]
    ...
    [SerializeField] private EnemyAttckType _attackType;
    ...
    {
        if (_isAttacking || _attackType == EnemyAttckType.Ranged)
        {
            for (int i = 0; i < _triggerZones.Length; i++)
            {
